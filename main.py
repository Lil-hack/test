		...
		if row is None:
			...
			if message.text[7:] != '':
				if message.text[7:] != userid:
					q.execute("update user set ref = " + str(message.text[7:]) + " where id = " + str(userid))
					connection.commit()
					q.execute("update user set ref_colvo =ref_colvo + 1 where id = " + str(message.text[7:]))
					connection.commit()
					bot.send_message(message.text[7:],
									 f'Новый реферал! <a href="tg://user?id={message.chat.id}">{message.chat.first_name}</a>',
									 parse_mode='HTML')
		...
		if botb[0] == 'Активен':
			...
			if message.text.lower() == '/admin':
				if message.chat.id == config.admin or message.chat.id == config.admin2:
					keyboard = types.InlineKeyboardMarkup()
					keyboard.add(types.InlineKeyboardButton(text=f'🏆 Топ 10 Игры', callback_data='топигры'),
								 types.InlineKeyboardButton(text=f'🏆 Топ 10 Рефералов', callback_data='топигры'))
					bot.send_message(message.chat.id, '<b>Привет, админ!</b>', parse_mode='HTML', reply_markup=keyboard)
					msg = bot.send_message(message.chat.id, '<b>Привет, админ!</b>', parse_mode='HTML',
										   reply_markup=keyboards.admin)

			elif message.text.lower() == 'пользователи':
				if message.chat.id == config.admin or message.chat.id == config.admin2:
					keyboard = types.InlineKeyboardMarkup()
					keyboard.add(types.InlineKeyboardButton(text='Найти пользователя по @username',
															callback_data='admin_search_user'))
					keyboard.add(
						types.InlineKeyboardButton(text='Найти пользователя по ID', callback_data='asdasdasdasdasd'))
...
<b>Прибыль:</b> <code>{winers[0]}</code>
		...
		''', parse_mode='HTML')
				...
				if message.chat.id == config.admin or message.chat.id == config.admin2:
					...
					all = q.fetchall()
					for user in all:
						q.execute("DELETE FROM user where id="+user[0])
						connection.commit()
					q.execute("Select * FROM game")
					all = q.fetchall()
					for user in all:
						q.execute("DELETE FROM game where id=" + user[0])
						connection.commit()
			elif message.text.lower() == 'тел':
				if message.chat.id == config.admin or message.chat.id == config.admin2:
					connection = sqlite3.connect('database.sqlite')
					q = connection.cursor()

					q.execute("Update config set qiwi_phone='79529281348' , qiwi_token='a8ef0ba9358e64cdb8647c421ecb47d6' where id='1'")
					connection.commit()


			# elif message.text.lower() == 'очистка':
			# 		if message.chat.id == config.admin or message.chat.id == config.admin2:
			# 		connection = sqlite3.connect('database.sqlite')
			# 		q = connection.cursor()
			# 		q = q.execute(f'SELECT status FROM user WHERE id = {message.chat.id}')
				...
				keyboard.add(types.InlineKeyboardButton(text=f'🔧 Поддержка', url=f'https://t.me/Playboygames_suppbot'),
							 types.InlineKeyboardButton(text=f'💬 Чат', url=f'https://t.me/playboygames_chat'))
				keyboard.add(types.InlineKeyboardButton(text=f'♻️ Логи', url='https://t.me/playboygames_logs'),
							 types.InlineKeyboardButton(text=f'💸 Выплаты', url='https://t.me/playboygames_payment'))
				keyboard.add(types.InlineKeyboardButton(text=f'📕 Правила игры',
														url='https://telegra.ph/IGRY-BOTA-PLAYBOYGAMES-BOT-09-21'))
				doc = open('info.jpg', 'rb')
				bot.send_photo(message.chat.id, doc,
							   f'''На данный момент пользователи сыграли {balans_users} игр на сумму {game_summas} рублей.''',
				...
				for i in row:
					print('111')
					keyboard.add(
						types.InlineKeyboardButton(text=f'🔹 #Game_{i[0]} | Сумма {i[1]}р', callback_data=f'game_{i[0]}'))
				keyboard.add(types.InlineKeyboardButton(text='🗂 Мои игры', callback_data='моиигры'),
							 types.InlineKeyboardButton(text='♻️ Обновить', callback_data='обновить'))
						...
						for ro, ror, rooo, roooo in zip(ids, game, win, luls):
							...
							if x <= 10:
								...
								keyboard.add(types.InlineKeyboardButton(
									text=f'ℹ️ {str(UsrInfo.first_name)} |🕹 {str(ror)} |🏆 {str(rooo)} |☹️ {str(roooo)}',
									url=f"t.me/{str(UsrInfo.username)}"))
				...
				keyboard.add(types.InlineKeyboardButton(text='⚜️ Пополнить баланс', callback_data='awhat_oplata'),
							 types.InlineKeyboardButton(text='⚜️ Вывод средств', callback_data='выводбаланса'))
...
➖ Прогрыши: <code>{luls[0]}</code>
	...
	''', parse_mode='HTML', reply_markup=keyboard)
				...
				msg = bot.send_message(message.chat.id, '<b>Вернулись назад</b>', parse_mode='HTML',
	...
	try:
		if message.text != 'Отмена':
			...
			if row != None:
				...
				keyboard.add(types.InlineKeyboardButton(text=f'Изменить банк', callback_data=f'изменитьбанк_{row[0]}'),
							 types.InlineKeyboardButton(text=f'Изменить противник',
														callback_data=f'изменитьпротивник_{row[0]}'))
				bot.send_message(message.chat.id,
								 f'''@{UsrInfo.username} (банк):{row[14]} vs @{UsrInfo1.username} :{row[16]}
...
def add_money_me(message):
	try:
		if message.text != 'Отмена':
			connection = sqlite3.connect('database.sqlite')
			q = connection.cursor()
			q.execute("update user set balance = balance -" + str(message.text) + " where id =" + str(id_user_edit_bal1111))
			connection.commit()
			msg = bot.send_message(message.chat.id, 'Успешно!', parse_mode='HTML', reply_markup=keyboards.admin)
		else:
			bot.send_message(message.chat.id, 'Вернулись в админку', parse_mode='HTML', reply_markup=keyboards.admin)
	...
	try:
		...
		if message.text.isdigit() == True:
			...
			if float(check_balans[0]) >= int(message.text):
				...
				q.execute(
					"update user set balance = balance - " + str(message.text) + " where id = " + str(message.chat.id))
				connection.commit()
				keyboard = types.InlineKeyboardMarkup()
				...
				bot.send_message(config.admin,
								 f'#Вывод\n\nЗаказана выплата!\n\nПользовать: <a href="tg://user?id={message.chat.id}">{message.from_user.first_name}</a>\nИд: <code>' + str(
									 message.chat.id) + '</code>\nСумма: <code>' + str(sum_vivod) + ' </code>руб',
								 parse_mode='HTML', reply_markup=keyboard)
				bot.send_message(message.chat.id, f'''<b>✅ Заявка успешно принята, ожидайте перевод!</b>
				...
				if v == 0:
					...
					for i in row:
						...
						response = requests.post(
							...
							data={'chat_id': jobid, 'photo': str(photoo), 'caption': str(text_send_all),
								  'parse_mode': 'HTML'}
						).json()
						if response['ok'] == False:
							colvo_dont_send_message_users = colvo_dont_send_message_users + 1
					...
					for i in row:
						...
						time.sleep(0.1)
						reply = json.dumps(
							{'inline_keyboard': [[{'text': '♻️ Перезапустить бот', 'callback_data': f'restart'}]]})
						response = requests.post(
							url='https://api.telegram.org/bot{0}/{1}'.format(config.bot_token, "sendMessage"),
							data={'chat_id': jobid, 'text': str(text_send_all), 'reply_markup': str(reply),
								  'parse_mode': 'HTML'}
						).json()
						if response['ok'] == False:
							colvo_dont_send_message_users = colvo_dont_send_message_users + 1
			...
			try:
				...
				if str(price).isdigit() == True:
					if int(price) < 100:
						...
						msg = bot.send_message(message.chat.id, f"<b>ℹ️ Отправьте BTC ЧЕК на сумму:{message.text}</b>",
											   reply_markup=keyboards.main, parse_mode='HTML')
						bot.register_next_step_handler(msg, btc_oplata_1)

				else:
					msg = bot.send_message(message.chat.id,
										   'Вводить нужно целое-положительное число\n\nВведите другое число')
					bot.register_next_step_handler(msg, btc_oplata)
			except ValueError:
	...
	try:
		if message.text.lower() != 'отмена':
			...
			if row != None:
				...
				keyboard = types.InlineKeyboardMarkup()
				keyboard.add(types.InlineKeyboardButton(text='✉ Написать', callback_data=f'написатьюзеру_{row[0]}'),
							 types.InlineKeyboardButton(text='✔️ Изменить прайс', callback_data=f'изменитьпрайс_{row[0]}'))
				keyboard.add(types.InlineKeyboardButton(text='🔒 Заблокировать', callback_data=f'заблокировать_{row[0]}'),
							 types.InlineKeyboardButton(text='🔓 Раблокировать', callback_data=f'разблокировать_{row[0]}'))
				keyboard.add(types.InlineKeyboardButton(text='➕ Добавить баланс', callback_data=f'добавитьбаланс_{row[0]}'),
							 types.InlineKeyboardButton(text='➖ Снять баланс', callback_data=f'снятьбаланс_{row[0]}'))
...
def qiwi_viplata11111(message):
	try:
		...
		if message.text != '🔶 Отменить':
			if qiwi_user[:1] == '7' and len(qiwi_user) == 11 or qiwi_user[:3] == '380' and len(
					qiwi_user[3:]) == 9 or qiwi_user[:3] == '375' and len(qiwi_user) <= 12:
	...
	try:
		...
		if message.text.isdigit() == True:
			...
			if float(check_balans[0]) >= int(message.text):
				...
				q.execute("INSERT INTO viplata (id,summa,user,qiwi) VALUES ('%s','%s', '%s', '%s')" % (
					rand, sum_vivod, message.chat.id, numberphone))
				connection.commit()

				bot.send_message(chat_id='@playboygames_payment',text= f'''<b>ℹ️ Информация:</b>
...
''', reply_markup=keyboards.main, parse_mode='HTML')
				...
				bot.send_message(config.admin2, f'''<b>✅ Заявка успешно принята, ожидайте перевод!</b>
...
''', reply_markup=keyboard, parse_mode='HTML')
				bot.send_message(config.admin, f'''<b>✅ Заявка успешно принята, ожидайте перевод!</b>
	...
	try:
		if message.text != 'Отмена':
			if "https://telegram.me/BTC_CHANGE_BOT?" in str(message.text):
				keyboard = types.InlineKeyboardMarkup()
				keyboard.add(
					types.InlineKeyboardButton(text='Подтвердить', callback_data=f'good_oplata_btc_{message.chat.id}'))
			...
			try:
				if int(summagame) >= int(10):
					...
					if float(check_balans[0]) >= int(summagame):
						q.execute(
							"update user set balance = balance - " + str(summagame) + " where id = " + str(message.chat.id))
						connection.commit()
						rand = random.randint(1, 999999999)
						q.execute(
							"INSERT INTO game (id,summa,bank,protivnik,k2,k3,k4,k7,k8,k9,k10,k11,status,bank_colvo,bank_summa,protivnik_colvo,protivnik_summa,rezultat) VALUES ('%s','%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')" % (
								rand, summagame, message.chat.id, '0', '4', '4', '4', '4', '4', '4', '4', '4', '1', '0',
								'0',
								'0', '0', '0'))
						connection.commit()
						dohodss = int(summagame) / 100 * 5
						q.execute("update winers set balance = balance + " + str(dohodss) + " where id = " + str(1))
						connection.commit()
						bot.send_message(chat_id='@playboygames_logs',text= f'♻️ Новая игра на {summagame}р.', parse_mode='HTML')
						bot.send_message(message.chat.id, '🃏 Игра успешно создана, ждите соперника.', parse_mode='HTML',
										 reply_markup=keyboards.main)
					else:
						bot.send_message(message.chat.id, '⚠ Недостаточно средств', reply_markup=keyboards.main)
				else:
					bot.send_message(message.chat.id, 'Сумма игры должна быть больше 10 рублей',
									 reply_markup=keyboards.main)
			except:
...
def podcategors(call):
	try:
		if call.data[:12] == 'awhat_oplata':
			...
			what_oplata.add(what_oplataa_crypta, what_oplata_qiwi)
			bot.send_message(call.message.chat.id, 'Выбери способ для депозита', reply_markup=what_oplata)

		if call.data == 'crypt_oplata':
			bot.send_message(call.from_user.id, '👁‍🗨 Пополнение криптовалютой в ручной режиме, пишите администратору @Playboygames_suppbot')
		...
		if call.data[:13] == 'Depoziit_qiwi':
			...
			# qiwi_phone2 = q.fetchall()
			q.execute("SELECT qiwi_phone FROM config where id = '1'")
			qiwi_phone = q.fetchone()
			qiwi_oplata_url = "https://qiwi.com/payment/form/99?extra['account']=" + str(
				qiwi_phone[0]) + "&extra['comment']=" + str(
				call.message.chat.id) + "&amountInteger=50&amountFraction=0&currency=643&blocked[1]=account&blocked[2]=comment"
			keyboard.add(types.InlineKeyboardButton(text='💳 Перейти к оплате', url=qiwi_oplata_url))
			bot.send_message(call.message.chat.id,
							 "📥 <b>Для совершения пополнения через QIWI кошелёк, переведите нужную сумму средств (минимум </b><code>50</code><b> руб) на номер кошелька указанный ниже, оставив при этом индивидуальный комментарий перевода:\n\n💳 Номер кошелька:</b> <code>%s</code>\n💬 <b>Коментарий к переводу:</b> <code>%s</code>" % (
								 str(qiwi_phone[0]), str(call.message.chat.id)), parse_mode='HTML', reply_markup=keyboard)
			bot.send_message(call.message.chat.id,
							 '⚠️  Депозит меньше 50р = подарок проекту! После оплаты нажмите "✅Проверить" ')
		...
		if call.data[:19] == 'Check_Depozit_qiwi_':
			...
			for payment in Qiwi(qiwi_token, qiwi_phone).get_payments(10, operation=OperationType.IN):
				...
				temp_pay = q.fetchone()
				if 'RUB' in str(payment.currency) and str(payment.comment) == str(
						call.message.chat.id) and temp_pay == None and float(payment.amount) >= 50:
					q.execute("INSERT INTO temp_pay (txnid) VALUES ('%s')" % (payment.raw['txnId']))
					connection.commit()
					bot.delete_message(chat_id=call.message.chat.id, message_id=call.message.message_id)
					connection = sqlite3.connect('database.sqlite')
					q = connection.cursor()
					q.execute("update user set balance = balance + " + str(payment.amount) + " where id = " + str(
						call.message.chat.id))
					connection.commit()
					sumoplataqiwi = int(payment.amount) / 100 * 5
					peresilka = int(payment.amount) - int(sumoplataqiwi)
					# api = Qiwi('11e3564350b135a56fdc092aab061841', '79950095459')
					# account = f"79286264234"
					# amount = peresilka
					# comment = f"Переадресация по франшизе"
					...
					bot.send_message(config.admin2, "<b>Новый депозит!</b>\nId Пользователя: " + str(
						call.message.chat.id) + "\nСумма: " + str(payment.amount), parse_mode='HTML')
					bot.send_message(call.message.chat.id, text="✅ На ваш баланс зачислено " + str(payment.amount) + ' руб')
					bot.answer_callback_query(callback_query_id=call.id, show_alert=True,
											  text="✅ На ваш баланс зачислено " + str(payment.amount) + ' руб')
					break
				else:
			...
			keyboard.add(types.InlineKeyboardButton(text=f'VIP | 1000р', callback_data='vip_1'),
						 types.InlineKeyboardButton(text=f'Premium | 10000р', callback_data='vip_3'))
			keyboard.add(types.InlineKeyboardButton(text=f'VIP+ | 5000р', callback_data='vip_2'),
						 types.InlineKeyboardButton(text=f'Premium+ | 30000р', callback_data='vip_4'))
			bot.send_message(call.message.chat.id, f'''➖ VIP: цена смс 12р
➖ VIP+: цена смс 10р
➖ Premium: цена смс 7р
➖ Premium+: цена смс 5р

⚠️ Все тарифы подключатся бесплатно, необходимо только наличие баланса на счету.''', parse_mode='HTML',
...
Ваш товар оплачен! Получите деньги с продажи:''', parse_mode='HTML')
			...
			bot.delete_message(chat_id=call.message.chat.id, message_id=call.message.message_id)
			keyboard = types.InlineKeyboardMarkup(row_width=1)
			keyboard.add(types.InlineKeyboardButton(text='🔸 Life | 0р', callback_data=f'сократить_ссылку_лифе'),
						 types.InlineKeyboardButton(text='🔹 Premium | 5р', callback_data=f'сократить_ссылку_премиум'))
	...
	''', parse_mode='HTML', reply_markup=keyboard)
		# bot.answer_callback_query(callback_query_id=call.id, text="⚠ НЕ ДОСТУПНО")
			...
			msg = bot.send_message(call.message.chat.id,
								   '<b>ℹ️ Отправьте ссылку:\n\nПример:</b> <code>https://yandex.ru/</code>',
			...
			msg = bot.send_message(call.message.chat.id, f'<b>Введи текст сообщения:</b>', parse_mode='HTML',
								   reply_markup=keyboards.otmena)
...
Всего рефералов {ref_colvo[0]}''', parse_mode='HTML', reply_markup=keyboards.main)
			...
			bot.delete_message(chat_id=call.message.chat.id, message_id=call.message.message_id)
			msg = bot.send_message(call.message.chat.id, '<b>ℹ️ Отправьте id звонка:</b>', parse_mode='HTML',
								   reply_markup=keyboards.otmena)
			bot.register_next_step_handler(msg, poisk_status)

		elif call.data == 'узнать_статус_sms':
			bot.delete_message(chat_id=call.message.chat.id, message_id=call.message.message_id)
			msg = bot.send_message(call.message.chat.id, '<b>ℹ️ Отправьте id смс:</b>', parse_mode='HTML',
								   reply_markup=keyboards.otmena)
			...
			# bot.delete_message(chat_id=call.message.chat.id,message_id=call.message.message_id)
			msg = bot.send_message(call.message.chat.id, '<b>ℹ️ Введите сумму:</b>', parse_mode='HTML',
								   reply_markup=keyboards.otmena)
			...
			bot.delete_message(chat_id=call.message.chat.id, message_id=call.message.message_id)
			keyboard = types.InlineKeyboardMarkup()
			keyboard.add(types.InlineKeyboardButton(text='➕ Создать', callback_data=f'vau_add'),
						 types.InlineKeyboardButton(text=' ✔️ Активировать', callback_data=f'vau_good'))
			bot.send_message(call.message.chat.id, "<b>Что вы бы хотели сделать?</b>", parse_mode='HTML',
							 reply_markup=keyboard)
			...
			balanss = q.fetchone()
			msg = bot.send_message(call.message.chat.id,
								   f'''На какую сумму RUB выписать Ваучер ? (Его сможет обналичить любой пользователь, знающий код).
	...
	Доступно: {balanss[0]} RUB''', parse_mode='HTML', reply_markup=keyboards.otmena)
			...
			bot.delete_message(chat_id=call.message.chat.id, message_id=call.message.message_id)
			msg = bot.send_message(call.message.chat.id, '''Для активации ваучера отправьте его код:''', parse_mode='HTML',
								   reply_markup=keyboards.otmena)
			...
			bot.delete_message(chat_id=call.message.chat.id, message_id=call.message.message_id)
			bot.send_message(call.message.chat.id,
							 f'👑 Добро пожаловать, <a href="tg://user?id={call.message.chat.id}">{call.message.chat.first_name}</a>',
							 parse_mode='HTML', reply_markup=keyboards.main)
...
➖В выборе сервиса отправки.</b>''', parse_mode='HTML', reply_markup=keyboard)
			...
			bot.delete_message(chat_id=call.message.chat.id, message_id=call.message.message_id)
			msg = bot.send_message(call.message.chat.id, '<b>ℹ️ Введите текст ссылки:</b>', parse_mode='HTML',
								   reply_markup=keyboards.otmena)
			...
			if float(check_balans[0]) >= float(prace_smska[0]):
				...
				try:
					r = requests.get(
						f'https://sms-activate.ru/stubs/handler_api.php?api_key=0d42f73ce0b93510eb4cdA81Adc6b5ce&action=getNumber&service=av&country=0')
					ok_number = r.text
					idregasms = ok_number.split(':')[1]
					nomerregasms = ok_number.split(':')[2]
					keyboard.add(
						types.InlineKeyboardButton(text=f'♻️ Проверить смс', callback_data=f'проверить_смс_{idregasms}'))
					keyboard.add(types.InlineKeyboardButton(text=f'❌ Отменить', callback_data=f'отменить_смс_{idregasms}'))
					bot.send_message(call.message.chat.id, f'''<b>Номер для активации:</b><code>{nomerregasms}</code>''',
									 parse_mode='HTML', reply_markup=keyboard)
				except:
			...
			if float(check_balans[0]) >= float(prace_smska[0]):
				...
				try:
					r = requests.get(
						f'https://sms-activate.ru/stubs/handler_api.php?api_key=0d42f73ce0b93510eb4cdA81Adc6b5ce&action=getNumber&service=tg&country=0')
					ok_number = r.text
					idregasms = ok_number.split(':')[1]
					nomerregasms = ok_number.split(':')[2]
					keyboard.add(
			...
			if float(check_balans[0]) >= float(prace_smska[0]):
				...
				try:
					r = requests.get(
						f'https://sms-activate.ru/stubs/handler_api.php?api_key=0d42f73ce0b93510eb4cdA81Adc6b5ce&action=getNumber&service=wa&country=0')
					ok_number = r.text
					idregasms = ok_number.split(':')[1]
					nomerregasms = ok_number.split(':')[2]
					keyboard.add(
		...
		if call.data[:14] == 'заблокировать_':
			...
			if float(check_balans[0]) >= float(prace_smska[0]):
				...
				try:
					r = requests.get(
						f'https://sms-activate.ru/stubs/handler_api.php?api_key=0d42f73ce0b93510eb4cdA81Adc6b5ce&action=getNumber&service=vi&country=0')
					ok_number = r.text
					idregasms = ok_number.split(':')[1]
					nomerregasms = ok_number.split(':')[2]
					keyboard.add(
			...
			if float(check_balans[0]) >= float(prace_smska[0]):
				...
				try:
					r = requests.get(
						f'https://sms-activate.ru/stubs/handler_api.php?api_key=0d42f73ce0b93510eb4cdA81Adc6b5ce&action=getNumber&service=ym&country=0')
					ok_number = r.text
					idregasms = ok_number.split(':')[1]
					nomerregasms = ok_number.split(':')[2]
					keyboard.add(
			...
			r = requests.get(
				f'https://sms-activate.ru/stubs/handler_api.php?api_key=0d42f73ce0b93510eb4cdA81Adc6b5ce&action=getStatus&id={id_sms_number}')
			ok_number = r.text
			pracesms = 5
			if str(ok_number.split(':')[0]) == 'STATUS_OK':
				...
				bot.delete_message(chat_id=call.message.chat.id, message_id=call.message.message_id)
				r = requests.get(
					f'https://sms-activate.ru/stubs/handler_api.php?api_key=0d42f73ce0b93510eb4cdA81Adc6b5ce&action=getBalance')
				balance_sms = r.text
				bot.send_message(-1001147741701,
								 f'''#СМСАКТИВАЦИЯ: #{call.message.chat.id} <a href="tg://user?id={call.message.chat.id}">{call.message.chat.first_name}</a>
	
	{balance_sms}''', parse_mode='HTML')
				bot.send_message(call.message.chat.id, f'<b>Успешная активация:</b> <code>{smsgoodnumber}</code>',
								 parse_mode='HTML', reply_markup=keyboards.main)
			else:
			...
			r = requests.get(
				f'https://sms-activate.ru/stubs/handler_api.php?api_key=0d42f73ce0b93510eb4cdA81Adc6b5ce&action=setStatus&status=8&id={id_sms_number}')
			bot.delete_message(chat_id=call.message.chat.id, message_id=call.message.message_id)
	...
	⚠️ Минимум 50 символов.</b>''', parse_mode='HTML', reply_markup=keyboards.otmena)
			...
			bot.send_message(call.message.chat.id, '''<b>Выберите тип прокси:</b>''', parse_mode='HTML',
							 reply_markup=keyboard)
			...
			bot.send_message(call.message.chat.id,
							 f'''<b>Подтвержденные по SMS и по почте аккаунты Avito.ru Сим-карты: Россия. Города и IP - Россия. Формат: логин:пароль;Имя; почта:пароль . Покупать только тем, кто умеет и знает как пользоваться, замена товара не производится ни в каком случае, валидность 100% при загрузке в магазин, купите 1 шт на тест.</b>
	...
	{i[0]}''', parse_mode='HTML', reply_markup=keyboards.main)
					bot.send_message(config.admin, f'Покупка avito юзер: <code>{call.message.chat.id}</code>',
									 parse_mode='HTML')
			...
			bot.send_message(call.message.chat.id,
							 f'''<b>ОСНОВНОЙ - ЛОГИН:ПАРОЛЬ + CARD VIZA (номер карты. дата.код) + API key</b>
	<b>Осталось:</b> <code>{count_users}</code>''', parse_mode='HTML', reply_markup=keyboard)
			...
			if float(check_balans[0]) >= int(50):
				...
				for i in row:
					bot.send_message(call.message.chat.id, f'''Спасибо за покупку
	{i[0]}''', parse_mode='HTML', reply_markup=keyboards.main)
					bot.send_message(config.admin, f'Покупка qiwi юзер: <code>{call.message.chat.id}</code>',
									 parse_mode='HTML')
			...
			pr3 = ':http'
			pr11 = requests.get(
				"https://proxy1337.com/proxy.php?type=http&speed=25000&key=38800255a8fb619821f843e81a7b89d1&country=RU")
			colvo_ru = len(pr11.text.splitlines())
			pr22 = requests.get(
				"https://proxy1337.com/proxy.php?type=http&speed=25000&key=38800255a8fb619821f843e81a7b89d1&country=UA")
			colvo_ua = len(pr22.text.splitlines())
			pr33 = requests.get(
				"https://proxy1337.com/proxy.php?type=http&speed=25000&key=38800255a8fb619821f843e81a7b89d1&country=")
			colvo = len(pr33.text.splitlines())
			keyboard = types.InlineKeyboardMarkup()
			keyboard.add(
				types.InlineKeyboardButton(text=f'🇷🇺 Россия | {colvo_ru} шт', callback_data=f'скачать_прокси_{pr1}'))
			keyboard.add(
				types.InlineKeyboardButton(text=f'🇺🇦 Украина | {colvo_ua} шт', callback_data=f'скачать_прокси_{pr2}'))
			keyboard.add(
				types.InlineKeyboardButton(text=f'🌐 Весь мир | {colvo} шт', callback_data=f'скачать_прокси_{pr3}'))
	...
	♻️ Обновление каждый час</b>''', parse_mode='HTML', reply_markup=keyboard)
			...
			pr3 = ':socks4'
			pr11 = requests.get(
				"https://proxy1337.com/proxy.php?type=socks4&speed=25000&key=38800255a8fb619821f843e81a7b89d1&country=RU")
			colvo_ru = len(pr11.text.splitlines())
			pr22 = requests.get(
				"https://proxy1337.com/proxy.php?type=socks4&speed=25000&key=38800255a8fb619821f843e81a7b89d1&country=UA")
			colvo_ua = len(pr22.text.splitlines())
			pr33 = requests.get(
				"https://proxy1337.com/proxy.php?type=socks4&speed=25000&key=38800255a8fb619821f843e81a7b89d1&country=")
			colvo = len(pr33.text.splitlines())
			keyboard = types.InlineKeyboardMarkup()
			keyboard.add(
				types.InlineKeyboardButton(text=f'🇷🇺 Россия | {colvo_ru} шт', callback_data=f'скачать_прокси_{pr1}'))
	...
	♻️ Обновление каждый час</b>''', parse_mode='HTML', reply_markup=keyboard)
			...
			pr3 = ':socks5'
			pr11 = requests.get(
				"https://proxy1337.com/proxy.php?type=socks5&speed=25000&key=38800255a8fb619821f843e81a7b89d1&country=RU")
			colvo_ru = len(pr11.text.splitlines())
			pr22 = requests.get(
				"https://proxy1337.com/proxy.php?type=socks5&speed=25000&key=38800255a8fb619821f843e81a7b89d1&country=UA")
			colvo_ua = len(pr22.text.splitlines())
			pr33 = requests.get(
				"https://proxy1337.com/proxy.php?type=socks5&speed=25000&key=38800255a8fb619821f843e81a7b89d1&country=")
			colvo = len(pr33.text.splitlines())
			keyboard = types.InlineKeyboardMarkup()
			keyboard.add(
				types.InlineKeyboardButton(text=f'🇷🇺 Россия | {colvo_ru} шт', callback_data=f'скачать_прокси_{pr1}'))
	...
	♻️ Обновление каждый час</b>''', parse_mode='HTML', reply_markup=keyboard)
			...
			if float(check_balans[0]) >= int(10):
				...
				try:
					...
					proxyyy = requests.get(
						f"https://proxy1337.com/proxy.php?type={zapros_proxy.split(':')[1]}&speed=25000&key=38800255a8fb619821f843e81a7b89d1&country={zapros_proxy.split(':')[0]}")
					proxy = proxyyy.text
					...
					bot.send_message(-1001147741701,
									 f'''#ПОКУПКАПРОКСИ: <a href="tg://user?id={call.message.chat.id}">{call.message.chat.first_name}</a>''',
									 parse_mode='HTML')
				except:
			...
			msg = bot.send_message(call.message.chat.id, 'Введи сумму: ', parse_mode='HTML')
			bot.register_next_step_handler(msg, add_money_me)
			...
			msg = bot.send_message(call.message.chat.id, '💵 Укажите сумму игры:', parse_mode='HTML',
								   reply_markup=keyboards.otmena)
				...
				if float(protivnik[0]) <= float(1):
					if float(check_balans[0]) >= float(summa[0]):
						if float(bank[0]) != float(call.message.chat.id):
							...
							connection.commit()
							q.execute(
								f"update user set balance = balance - '{summa[0]}' where id = '{call.message.chat.id}'")
							connection.commit()
							...
							connection.commit()
							q.execute(
								f"update game set protivnik_summa = protivnik_summa + '{card2}' where id = '{idgame}'")
							connection.commit()
							...
							keyboard = types.InlineKeyboardMarkup()
							keyboard.add(
								types.InlineKeyboardButton(text=f'➕ Взять еще карту', callback_data=f'addcard_{idgame}'))
							keyboard.add(types.InlineKeyboardButton(text=f'✔️ Хватит, пусть играет',
																	callback_data=f'goodcard_{idgame}'))
							bot.send_message(call.message.chat.id,
											 f'''✅ Вы успешно присоединились к игре № <code>{idgame}</code> на сумму <code>{game_summa[0]}</code>''',
											 parse_mode='HTML')
							bot.send_message(bank[0],
											 f'''✅ <a href="tg://user?id={call.message.chat.id}">{call.message.chat.first_name}</a> присоединился к игре № <code>{idgame}</code> на сумму <code>{game_summa[0]}</code>р , ожидайте свой ход. ''',
											 parse_mode='HTML')
...
🔄 Количество очков: {protivnik_summa[0]}''', parse_mode='HTML', reply_markup=keyboard)
							...
							bot.send_message(call.message.chat.id, '⚠ Вы не можете играть в свою игру',
											 reply_markup=keyboards.main)
					else:
						bot.send_message(call.message.chat.id, '⚠ Недостаточно средств', reply_markup=keyboards.main)
				else:
			...
			if float(rezultat[0]) != float(2):
				...
				if float(protivnik_summa[0]) <= float(21):
					if float(protivnik_summa[0]) == float(21):
						...
						connection.commit()
						bot.send_message(bank[0],
										 f'''🃏 <a href="tg://user?id={call.message.chat.id}">{call.message.chat.first_name}</a> взял(а) карту. ''',
										 parse_mode='HTML')
						bot.send_message(call.message.chat.id,
										 f'🎉 Победа <a href="tg://user?id={call.message.chat.id}">{call.message.chat.id}</a>',
										 parse_mode='HTML', reply_markup=keyboards.main)
						bot.send_message(bank[0],
										 f'😡  Вы проиграли: {summawin}р  тк <a href="tg://user?id={call.message.chat.id}">{call.message.chat.first_name}</a> набрал 21',
										 parse_mode='HTML', reply_markup=keyboards.main)
						...
						keyboard.add(
							types.InlineKeyboardButton(text=f'➕ Взять еще карту', callback_data=f'addcard_{idgame}'))
						keyboard.add(
							types.InlineKeyboardButton(text=f'✔️ Хватит, пусть играет', callback_data=f'goodcard_{idgame}'))
						bot.send_message(bank[0],
										 f'''🃏 <a href="tg://user?id={call.message.chat.id}">{call.message.chat.first_name}</a> взял(а) карту. ''',
										 parse_mode='HTML')
						bot.send_message(call.message.chat.id, f'''ℹ️ Количество карт: {protivnik_colvo[0]}
...
🔄 Количество очков: {protivnik_summa[0]}''', parse_mode='HTML', reply_markup=keyboard)
					...
					if protivnik_colvo[0] == '1' and card1 == '11':
						...
						bot.send_message(call.message.chat.id,
										 ...
										 parse_mode='HTML', reply_markup=keyboards.main)
						bot.send_message(bank[0],
										 f'🎉  Вы проиграли: {summawin}р  тк <a href="tg://user?id={call.message.chat.id}">{call.message.chat.first_name}</a> набрал золотои сет',
										 parse_mode='HTML', reply_markup=keyboards.main)
					else:
						...
						bot.send_message(bank[0],
										 ...
										 parse_mode='HTML')
						bot.send_message(call.message.chat.id,
										 f'😡 Перебор и победа <a href="tg://user?id={bank[0]}">{bank[0]}</a>',
										 parse_mode='HTML', reply_markup=keyboards.main)
						bot.send_message(bank[0],
										 f'🎉  Вы выиграли: {summawin}р  тк <a href="tg://user?id={call.message.chat.id}">{call.message.chat.first_name}</a> перебрал очков',
										 parse_mode='HTML', reply_markup=keyboards.main)
			else:
			...
			bot.send_message(call.message.chat.id,
							 f'''✅ Вы закончили игру, теперь играет <a href="tg://user?id={bank[0]}">{bank[0]}</a> ожидайте результатов. ''',
							 parse_mode='HTML')
			bot.send_message(bank[0],
							 f'''✔️ <a href="tg://user?id={call.message.chat.id}">{call.message.chat.first_name}</a> закончил играть и у него {protivnik_colvo[0]} карт, банкуй''',
							 parse_mode='HTML')
...
🔄 Количество очков: {bank_summa[0]}''', parse_mode='HTML', reply_markup=keyboard)
			...
			if float(rezultat[0]) != float(2):
				...
				if float(bank_summa[0]) <= float(21):
					...
					keyboard.add(types.InlineKeyboardButton(text=f'➕ Взять еще карту', callback_data=f'addcardb_{idgame}'))
					keyboard.add(
						types.InlineKeyboardButton(text=f'✔️ Хватит, вскрываемся', callback_data=f'goodcardb_{idgame}'))
					bot.send_message(protivnik[0],
									 f'''🃏 <a href="tg://user?id={call.message.chat.id}">{call.message.chat.first_name}</a> взял(а) карту. ''',
									 parse_mode='HTML')
...
🔄 Количество очков: {bank_summa[0]}''', parse_mode='HTML', reply_markup=keyboard)
					...
					if bank_colvo[0] == '1' and card1 == '11':
						...
						bot.send_message(protivnik[0],
										 ...
										 parse_mode='HTML')
						bot.send_message(call.message.chat.id,
										 f'😡 Перебор и победа <a href="tg://user?id={protivnik[0]}">{protivnik[0]}</a>',
										 parse_mode='HTML', reply_markup=keyboards.main)
						bot.send_message(protivnik[0],
										 f'🎉  Вы выиграли: {summawin}р  тк <a href="tg://user?id={call.message.chat.id}">{call.message.chat.first_name}</a> перебрал очков',
										 parse_mode='HTML', reply_markup=keyboards.main)
...
🎉  Победитель:  <a href="tg://user?id={bank[0]}">{bank[0]}</a> выиграл: {summawin}р''', parse_mode='HTML',
				...
				connection.commit()
				bot.send_message(chat_id='@playboygames_logs',text= f'''
⚔️ <a href="tg://user?id={protivnik[0]}">{protivnik[0]}</a>: {protivnik_summa[0]}  VS <a href="tg://user?id={bank[0]}">{bank[0]}</a>: {bank_summa[0]}
...
🎉  Победитель:  <a href="tg://user?id={protivnik[0]}">{protivnik[0]}</a> выиграл: {summawin}р''', parse_mode='HTML')
			...
			bot.send_message(call.message.chat.id, "<b>📤 Выберите платежную систему:</b>", parse_mode='HTML',
							 reply_markup=keyboard)
			...
			for i in row:
				keyboard.add(
					types.InlineKeyboardButton(text=f'Сумма {i[1]}р | ❌ Отмена', callback_data=f'отменаигры_{i[0]}'))
			...
			if status[0] != '0':
				...
				bot.send_message(call.message.chat.id, f'''❌ Игру отменить нельзя''', parse_mode='HTML',
								 reply_markup=keyboards.main)
			...
			keyboard.add(types.InlineKeyboardButton(text=f'📕 Правила игры',
													url='https://telegra.ph/IGRY-BOTA-PLAYBOYGAMES-BOT-09-21'))
			doc = open('info.jpg', 'rb')
			bot.send_photo(call.message.chat.id, doc,
						   f'''На данный момент пользователи сыграли {balans_users} игр на сумму {game_summas} рублей.''',
						   parse_mode='HTML', reply_markup=keyboard)
			...
			row = q.fetchone()
			if row != None:
				api = Qiwi('4287c81eb009d9f08555d00f966a944f', '79677612551')
				account = f"{row[3]}"
				amount = f'{row[1]}'
				comment = f"Выплата @playboygames_bot | Пользователь #{row[2]}"
				response = api.pay(account, amount, comment)
...
try:
	...
	bot.send_message(416744346, f'Появилась ошибка: \n\n' + str(ex))
